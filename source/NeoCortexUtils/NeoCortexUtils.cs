...
namespace NeoCortex
{
    ...
    public class NeoCortexUtils
    {
        ...
        /// Draws legends for a heatmap image indicating the colors and their corresponding values.
        /// <param name="g">The graphics object on which to draw the legends.</param>
        /// <param name="bmpWidth">The width of the bitmap.</param>
        /// <param name="bmpHeight">The height of the bitmap.</param>
        /// <param name="redStart">The starting value for the red color.</param>
        /// <param name="yellowMiddle">The middle value for the yellow color.</param>
        /// <param name="greenStart">The starting value for the green color.</param>

        private static void DrawLegends(Graphics g, int bmpWidth, int bmpHeight, decimal redStart, decimal yellowMiddle, decimal greenStart)
        {
            // Define legend properties
            int legendWidth = 110;
            int legendHeight = 110;
            int legendX = bmpWidth - legendWidth - 10;
            int legendY = bmpHeight - legendHeight - 10;

            // Draw color gradient legend
            using (SolidBrush redBrush = new SolidBrush(Color.Red))
            using (SolidBrush greenBrush = new SolidBrush(Color.Green))
            {
                g.FillRectangle(redBrush, legendX, legendY, legendWidth / 2, legendHeight);
                g.FillRectangle(greenBrush, legendX + legendWidth / 2, legendY, legendWidth / 2, legendHeight);
            }

            // Calculate the y-coordinate for legend text
            int textY = legendY + legendHeight + 5;
